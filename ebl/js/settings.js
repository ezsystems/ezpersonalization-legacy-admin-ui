...
function initializeSolutionAndItemTypes() {
  ...
  if (solution === 'EXTENDED') {
    ...
    for (var i in mandatorInfo.itemTypeConfiguration.types) {
      ...
      var d = t.description + ' (' + t.id + ') ' + generateContentTypeOrLanguageString(t);
      var selectedTxt = '';
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
function generateContentTypeOrLanguageString(_type) {
  var OPEN = '[';
  var CLOSE = ']';
  var PIPE = '|';
  ...
  //fill content
  if (_type.language != null) {
    if (contentTypesOrLanguage.length > 0) {
      contentTypesOrLanguage += PIPE;
    }
    contentTypesOrLanguage += _type.language;
  }

  //close content
  if (contentTypesOrLanguage.length > 0) {
    contentTypesOrLanguage += CLOSE;
  }
  return contentTypesOrLanguage;
}
...
function renderScenario() {
  ...
  json = {
    standardFilterSet: scenarioDao.standardFilterSet
  };
  ...
  json = {
    profileFilterSet: scenarioDao.profileFilterSet
  };
