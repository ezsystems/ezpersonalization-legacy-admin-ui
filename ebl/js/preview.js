 ...
 var initialize = function () {
   ...
   $('#custom_parameters').keyup(function () {
     getCallId();
   });
 ...
 function getCallSrc() {
   ...
   var scriptSrc = recoHost + '/api/v2/' + customerID + '/' + userId + '/' + reference_code + '.jsonp';
   var items = $('#context_items');
   ...
   scriptSrc += '&attribute=title&attribute=categorypath&attribute=price';

   if (customParameters != null && customParameters.trim() !== '') {
     if (customParameters[0] === '&') {
       scriptSrc += '&' + customParameters.substring(1).trim();
     } else {
